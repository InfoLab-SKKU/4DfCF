...
from .data_preprocess_and_load.datasets import S1200, ABCD, UKB, Dummy, NYU, ADHD_all
...
class fMRIDataModule(pl.LightningDataModule):
    def __init__(self, **kwargs):
        ...
        if self.hparams.pretraining:
            split_dir_path = f'/home/researchsrv1/chris/demo/codes/SwiFT_1/SwiFT/data/splits/{self.hparams.dataset_name}/pretraining'
        else:
            split_dir_path = f'/home/researchsrv1/chris/demo/codes/SwiFT_1/SwiFT/data/splits/{self.hparams.dataset_name}'
        os.makedirs(split_dir_path, exist_ok=True)

        # print(os.path.exists(split_dir_path))
    ...
    def get_dataset(self):
        if self.hparams.dataset_name == "Dummy":
            ...
            return UKB
        elif self.hparams.dataset_name == 'NYU':
            return NYU
        elif self.hparams.dataset_name == 'ADHD_all':
            return ADHD_all
        else:
    ...
    def determine_split_randomly(self, S):
        ...
        S_test = np.setdiff1d(S, np.concatenate(
            [S_train, S_val]))  # np.setdiff1d(np.arange(S), np.concatenate([S_train, S_val]))
    ...
    def make_subject_dict(self):
        ...
        if self.hparams.dataset_name == "S1200":
            ...
            meta_data_residual = pd.read_csv(
                os.path.join(self.hparams.image_path, "metadata", "HCP_1200_precise_age.csv"))
            ...
            if self.hparams.downstream_task == 'sex':
                ...
                task_name = 'nihtbx_totalcomp_uncorrected'
            else:
            ...
            subject_list = [subj[next((i for i, c in enumerate(subj) if c.isdigit() and c != '0'), 0):15] for subj in
                            os.listdir(img_root)]
            # subject_list = [subj[8:15] for subj in os.listdir(img_root)]
            # print(subject_list)

            meta_data = pd.read_csv(os.path.join(self.hparams.image_path, "metadata", "NYU_phenotypic.csv"))
            if self.hparams.downstream_task == 'DX':
                task_name = 'DX'
            elif self.hparams.downstream_task == 'age':
                task_name = 'age'
            elif self.hparams.downstream_task == 'int_total':
            ...
            if self.hparams.downstream_task == 'sex':
                meta_task = meta_data[['ScanDir ID', task_name]].dropna()
            elif self.hparams.downstream_task == 'DX':
                meta_task = meta_data[['ScanDir ID', task_name]]
            else:
                meta_task = meta_data[['subjectkey', task_name, 'sex']].dropna()

            meta_task['DX'] = meta_task['DX'].apply(lambda x: 1 if x >= 1 else x)
            meta_task['ScanDir ID'] = meta_task['ScanDir ID'].astype(str)
            # print(meta_task['ScanDir ID'])
            # print(subject_list)
            # print(meta_task['ScanDir ID'].values)

            for subject in subject_list:
                # print(int(subject))

                if subject in meta_task['ScanDir ID'].values:
                    # print("aaaaa")
                    target = meta_task[meta_task["ScanDir ID"] == subject][task_name].values[0]
                    # print(target)
                    # DX = meta_task[meta_task["ScanDir ID"] == subject]["DX"].values[0]
                    final_dict[subject] = [target]
            ...
            meta_data = pd.read_csv(os.path.join(self.hparams.image_path, "metadata", "ADHD_all_phenotypic.csv"))
            if self.hparams.downstream_task == 'DX':
                task_name = 'DX'
            elif self.hparams.downstream_task == 'age':
                task_name = 'age'
            ...
            if self.hparams.downstream_task == 'sex':
                ...
                task_name = 'age'
            elif self.hparams.downstream_task == 'int_fluid':
                task_name = 'fluid'
            else:
                raise ValueError('downstream task not supported')
    ...
    def add_data_specific_args(cls, parent_parser: ArgumentParser, **kwargs) -> ArgumentParser:
        ...
        group.add_argument("--label_scaling_method", default="standardization", choices=["minmax", "standardization"],
                           help="label normalization strategy for a regression task (mean and std are automatically calculated using train set)")
        ...
        group.add_argument("--bad_subj_path", default=None,
                           help="path to txt file that contains subjects with bad fMRI quality")
        ...
        group.add_argument("--img_size", nargs="+", default=[96, 96, 96, 20], type=int,
                           help="image size (adjust the fourth dimension according to your --sequence_length argument)")
        group.add_argument("--sequence_length", type=int, default=20)
        group.add_argument("--stride_between_seq", type=int, default=1,
                           help="skip some fMRI volumes between fMRI sub-sequences")
        group.add_argument("--stride_within_seq", type=int, default=1,
                           help="skip some fMRI volumes within fMRI sub-sequences")
        ...
        group.add_argument("--limit_training_samples", type=int, default=None,
                           help="use if you want to limit training samples")
        return parser
...
#         elif self.hparams.dataset_name == 'UKB':
#             return UKB
#         elif self.hparams.dataset_name == 'ADHD':
#             return ADHD
#         else:
#             raise NotImplementedError
#
