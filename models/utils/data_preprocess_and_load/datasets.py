...
class NYU(BaseDataset):
    ...
    def _set_data(self, root, subject_dict):
        ...
        for i, subject_name in enumerate(subject_dict):
            ...
            if len(subject_name) == 5:
                subject_name_file = subject_name.zfill(7)
            else:
                subject_name_file = subject_name
            target = subject_dict[subject_name]

            # print(target)

            subject_path = os.path.join(img_root, 'sfnwmrda' + subject_name_file + '_session_1_rest_1')

            num_frames = len(os.listdir(subject_path)) - 2  # voxel mean & std
            session_duration = num_frames - self.sample_duration + 1

            for start_frame in range(0, session_duration, self.stride):
                data_tuple = (i, subject_name, subject_path, start_frame, self.stride, num_frames, target)
                data.append(data_tuple)
    ...
    def __getitem__(self, index):
        _, subject_name, subject_path, start_frame, sequence_length, num_frames, target = self.data[index]
        # age = self.label_dict[age] if isinstance(age, str) else age.float()
        ...
        if self.contrastive:
            ...
            y = y.permute(0, 4, 1, 2, 3)
            # ABCD image shape: 79, 97, 85
            y = torch.nn.functional.pad(y, (37, 36, 33, 30, 38, 38), value=background_value)[:96, :96, :96, :96,
                :96]  # adjust this padding level according to your data
            y = y.permute(0, 2, 3, 4, 1)

            background_value = rand_y.flatten()[0]
            rand_y = rand_y.permute(0, 4, 1, 2, 3)
            # ABCD image shape: 79, 97, 85
            rand_y = torch.nn.functional.pad(rand_y, (37, 36, 33, 30, 38, 38), value=background_value)[:96, :96, :96, :96,
                :96]  # adjust this padding level according to your data
            rand_y = rand_y.permute(0, 2, 3, 4, 1)
...
class ADHD_all(BaseDataset):
    ...
    def _set_data(self, root, subject_dict):
        ...
        for i, subject_name in enumerate(subject_dict):
            ...
            subject_path = os.path.join(img_root, 'sfnwmrda' + subject_name_file + '_session')
...
#                 :]  # adjust this padding level according to your data
#             y = y.permute(0, 2, 3, 4, 1)
#
#             background_value = rand_y.flatten()[0]
#             rand_y = rand_y.permute(0, 4, 1, 2, 3)
#             # ABCD image shape: 79, 97, 85
#             rand_y = torch.nn.functional.pad(rand_y, (6, 5, 0, 0, 9, 8), value=background_value)[:, :, :, :96,
#                      :]  # adjust this padding level according to your data
#             rand_y = rand_y.permute(0, 2, 3, 4, 1)
#
#             return {
...
#                 # background value = 0
#                 # minmax scaled in brain (0~1)
#                 y = torch.nn.functional.pad(y, (0, 1, 0, 0, 0, 0),
#                                             value=background_value)  # adjust this padding level according to your data
#             y = y.permute(0, 2, 3, 4, 1)
#
#             return {
#                 "fmri_sequence": y,
